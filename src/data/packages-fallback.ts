...
export interface DisplayPackage {
  kind: 'service' | 'maintenance' | 'addon' | 'fix-service';
  slug: string;
  name: string;
  description: string;
  priceCents: number;
...
export const fallbackPackages: DisplayPackage[] = [
  ...
  },
  {
    ...
    slug: 'magazin-online',
    name: 'Magazin online',
    description: 'Magazin online care chiar vinde, nu doar arată bine. De la câteva produse la mii.',
    priceCents: 250000,
    currency: 'EUR',
    interval: 'one_time',
    pricing: 'from',
  ...
  // Lucrări punctuale la preț fix (apar pe /servicii-rapide, NU pe /pachete).
  // Prețuri de pornire, indicative — [confirmă: prețurile finale, editabile din admin].
  {
    kind: 'fix-service',
