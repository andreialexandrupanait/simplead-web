...
import { inspectDomain } from '../../../lib/server/tools/dns-inspector';
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  try {
    const result = await inspectDomain(parsed.data);
    return json({ ok: true, result });
  } catch (err) {
    console.warn('[tools/dns] inspecția a eșuat:', err);
    return json({ ok: false, error: 'Inspecția a eșuat. Încearcă din nou.' }, 502);
  }
};
